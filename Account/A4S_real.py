...
class Account4s(object):
    ...
    def UpdateAccountInfo(self):
        sliceingInfo=self.aSliceingInfo
        for holding in self.sPosition:
            code,openTime,openValue,Share,Price=holding
            valueBeforeUpdate=Price*Share
            Info4code=sliceingInfo[self.hCodeIndex[code]]
            RF_Rate=Info4code[-1]
    ...
    def sliceingInfoAccepter(self):
        while True:
            self.aSliceingInfo, self.time = yield  # 由BackTest类传递过来
            #每收到一个slicingInfo都第一时间更新账户情况
            self.UpdateAccountInfo()
            hCodeWeight=self.StrategyIntance.planGenner.send(self.aSliceingInfo, self.time,self.sPosition)
            if hCodeWeight:#不为空的话说明有新的持仓计划
                self.AdjustWarehouseAsPlan(hCodeWeight,self.time[:8])#time的前8位是date
    ...
    def sell4CertainAmount(self,code,AmountExpectToSell,date):#在更新完今天仓位后运行
        try:
            priceNow = self.hSliceingInfo[code]
    ...
    def getHCodeWeightNow(self):
        ...
        for position in self.sPosition:
